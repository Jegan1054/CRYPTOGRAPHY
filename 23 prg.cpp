...
void counter_encrypt(int* plaintext_blocks, int* ciphertext_blocks, int num_blocks, int counter, int sk1, int sk2) {
    for (int i = 0; i < num_blocks; i++) {
        int keystream = sdes_encrypt(counter, sk1, sk2);
        ciphertext_blocks[i] = plaintext_blocks[i] ^ keystream;
        counter++;
    }
}
void counter_decrypt(int* ciphertext_blocks, int* plaintext_blocks, int num_blocks, int counter, int sk1, int sk2) {
    for (int i = 0; i < num_blocks; i++) {
        int keystream = sdes_encrypt(counter, sk1, sk2);
        plaintext_blocks[i] = ciphertext_blocks[i] ^ keystream;
        counter++;
    }
}
...
int main() {
    ...
    int plaintext_blocks[] = {0b00000001, 0b00000010, 0b00000100};
    int num_blocks = 3;
    ...
    printf("Encrypting...\n");
    counter_encrypt(plaintext_blocks, ciphertext_blocks, num_blocks, counter, sk1, sk2);
    printf("Ciphertext: ");
    for (int i = 0; i < num_blocks; i++) {
    ...
    printf("Decrypting...\n");
    counter_decrypt(ciphertext_blocks, decrypted_blocks, num_blocks, counter, sk1, sk2);
